	...
	public void tick(IBlockState state, World worldIn, BlockPos pos, Random random) {
		if (!worldIn.isRemote) {
			...
			if (!func_196383_a(worldIn, pos)) {
				worldIn.setBlockState(pos, ModBlocks.KATHARIAN_DIRT.getDefaultState());
			} else {
				if (worldIn.getLight(pos.up()) >= 9) {
					for (int i = 0; i < 4; ++i) {
						...
						if (worldIn.getBlockState(blockpos).getBlock() == ModBlocks.KATHARIAN_DIRT && func_196384_b(worldIn, blockpos) && worldIn.isAirBlock(blockpos.up())) {
