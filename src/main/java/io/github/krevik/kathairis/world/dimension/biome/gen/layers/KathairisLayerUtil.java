 ...
 * @author Krevik
 */
public class KathairisLayerUtil {
	...
	public static <T extends IArea, C extends IContextExtended<T>> ImmutableList<IAreaFactory<T>> buildOverworldProcedure(WorldType worldTypeIn, OverworldGenSettings settings, LongFunction<C> contextFactory) {
		...
		lvt_7_1_ = GenLayerKathairisRiver.INSTANCE.apply((IContextExtended) contextFactory.apply(1L), lvt_7_1_);
		...
		lvt_8_1_ = GenLayerKathairisRiverMix.INSTANCE.apply((IContextExtended) contextFactory.apply(100L), lvt_8_1_, lvt_7_1_);
	...
	private static <T extends IArea, C extends IContextExtended<T>> IAreaFactory<T> getBiomeLayer(IAreaFactory<T> parentLayer,
		...
		parentLayer = (new GenLayerKathairisBiome(chunkSettings)).apply((IContextExtended) contextFactory.apply(200L), parentLayer);
