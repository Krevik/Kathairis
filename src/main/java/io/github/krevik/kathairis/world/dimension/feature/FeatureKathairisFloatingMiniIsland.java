 ...
 * @author Krevik
 */
public class FeatureKathairisFloatingMiniIsland extends Feature<NoFeatureConfig> {
	...
	public boolean place(IWorld world, IChunkGenerator<? extends IChunkGenSettings> c, Random rand, BlockPos pos, NoFeatureConfig p_212245_5_) {
		if (rand.nextInt(200) == 0) {
			IBlockState cloudState = rand.nextInt(2) == 0 ? YELLOW_CLOUD.getDefaultState() : BLUE_CLOUD.getDefaultState();
			...
			for (int x = -radiusOfGrassPlacing / 2; x <= radiusOfGrassPlacing / 2; x++) {
				for (int z = -radiusOfGrassPlacing / 2; z <= radiusOfGrassPlacing / 2; z++) {
					if ((x * x) + (z * z) <= (radiusOfGrassPlacing / 2 * radiusOfGrassPlacing / 2)) {
						...
						world.setBlockState(tmp, KATHAIRIS_GRASS.getDefaultState(), 2);
						BlockPos upperPos = tmp.up();
						if (rand.nextInt(5) == 2) {
							world.setBlockState(upperPos, KATHAIRIS_MINI_GRASS.getDefaultState(), 2);
						}
