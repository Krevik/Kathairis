...
public class SandstormCommand extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length <= 0) {
			throw new WrongUsageException("/sandstorm <time>|<true/false>");
		} else {
				World world = sender.getEntityWorld();
				String word1 = args[0];
				if (!world.isRemote) {
					KathairisDataStorage data = KathairisDataStorage.getDataInstance(world);
					if (data != null) {
						if (word1 != null) {
							if (word1.equalsIgnoreCase("true") || word1.equalsIgnoreCase("false")) {
								if (word1.equalsIgnoreCase("false")) {
									...
									notifyCommandListener(sender, this, "Sandstorm time set to: " + 0, 0, sender.getName());
								}
								if (word1.equalsIgnoreCase("true")) {
									...
									KathairisPacketHandler.CHANNEL.sendToAll(message2);
									notifyCommandListener(sender, this, "Sandstorm time set to: " + time, 0, sender.getName());
								}
							} else {
						...
						throw new WrongUsageException("Kathairis data storage is not loaded!");
					}
			/*if(!world.isRemote) {
