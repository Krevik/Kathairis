...
public class BlockMysticDoors extends BaseBlock
{
    ...
    public BlockMysticDoors(String Name, Material material, CreativeTabs tab, float hardness1, float resistance, SoundType soundType)
    {
    ...
    private Item getItem()
    {
    	...
    	}else if(this==KCore.Adamantium_Door) {
    		return Item.getItemFromBlock(KCore.Adamantium_Door);
    	}else {
    		return Item.getItemFromBlock(KCore.Mystic_Wood_Door);
    	}
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
    {
        ...
        if (player.capabilities.isCreativeMode && state.getValue(HALF) == BlockMysticDoors.EnumDoorHalf.UPPER && worldIn.getBlockState(blockpos).getBlock() == this)
        {
            worldIn.setBlockToAir(blockpos);
        }

        if (state.getValue(HALF) == BlockMysticDoors.EnumDoorHalf.LOWER && worldIn.getBlockState(blockpos1).getBlock() == this)
        {
    ...
    public int getMetaFromState(IBlockState state)
    {
        ...
        if (state.getValue(HALF) == BlockMysticDoors.EnumDoorHalf.UPPER)
        {
            ...
            if (state.getValue(HINGE) == BlockMysticDoors.EnumHingePosition.RIGHT)
            {
                i |= 1;
            }
